  ...
  function initializeStarRating() {
    ...
    starContainers.forEach((container) => {
      ...
      stars.forEach((star, index) => {
        ...
        } else if (isHalfStar && index === fullStars) {
      ...
      // Reset to original rating on mouse leave
      container.addEventListener("mouseleave", () => {
        const currentRating = parseFloat(container.dataset.rating) || rating;
        const isHalfCurrentStar = currentRating % 1 !== 0;
        const fullCurrentStars = Math.floor(currentRating);

        stars.forEach((star, index) => {
          if (index < fullCurrentStars) {
            star.className = "fas fa-star";
          } else if (isHalfCurrentStar && index === fullCurrentStars) {
            star.className = "fas fa-star-half-alt";
          } else {
            star.className = "far fa-star";
          }
...
function initializeFilterSection() {
  const sortDropdown = document.querySelector(
    '.filter-section button[title="Sort Products"]'
  );
  const priceRangeDropdown = document.querySelector(
    '.filter-section button[title="Filter by Price"]'
  );
    ...
    dropdownMenu.addEventListener("click", (e) => {
      if (e.target.classList.contains("dropdown-item")) {
        ...
        products.sort((a, b) => {
          const getPrice = (el) =>
            parseFloat(
              el.querySelector(".current-price").textContent.replace("$", "")
            );
          const getRating = (el) =>
            parseInt(el.querySelector(".stars").dataset.rating);
          const getReviews = (el) =>
            parseInt(
              el.querySelector(".review-count").textContent.match(/\d+/)[0]
            );
      ...
      if (e.target.classList.contains("dropdown-item")) {
        ...
        Array.from(productsGrid.children).forEach((product) => {
          const price = parseFloat(
            product.querySelector(".current-price").textContent.replace("$", "")
          );
